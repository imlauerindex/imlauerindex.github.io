---
title: "Configurar tu Android para que automáticamente prenda al conectarlo al cargador"
...
Ahora en opciones del programador (developer options) habilitá `usb-debugging, oem unlock, stay awake` instalá platform-tools adb y fastboot, ejecutá adb devices conectá el celular a la PC con USB, te saldrá un mensaje que te pedirá autorización en tu celular, a veces no sale tenés que habilitar y deshabilitar usb debugging hasta que funcione. Ahora ejecutá adb devices y deberías poder ver el celular luego `adb reboot bootloader` ahora `sudo fastboot devices` deberías poder ver el celular si no lo ves desconectá el cable rapido y volvelo a conectar hasta que aparezca, ahora ejecutá:   
